...
Example programs showing how to build encrypted applications on Solana with [Arcium](https://arcium.com).
...
> Every `arcium test` runs real Arx nodes in Docker on your machine — no mocked computations.
...
| [RPS vs House](./rock_paper_scissors/against-house/) | Stateless random | Beginner | No | MXE-generated house move | Comparing with coinflip |
...
**Core path**: Coinflip → Voting → Sealed-Bid Auction → Blackjack → Ed25519

**Optional branches**:
- After Coinflip: [RPS vs House](./rock_paper_scissors/against-house/) → [RPS vs Player](./rock_paper_scissors/against-player/) (randomness and hidden-input variants)
- After Voting: [Medical Records](./share_medical_records/) (re-encryption pattern)
...
| Docker | Any recent | [docker.com](https://docs.docker.com/get-docker/) (Arx nodes run in containers) |
...
```
example/
  encrypted-ixs/src/lib.rs   # Arcis circuit (encrypted instruction)
  programs/*/src/lib.rs       # Solana program (Anchor)
  tests/*.ts                  # End-to-end test
  Arcium.toml                 # Localnet Arcium config
...
`arcium build` compiles circuits and syncs program IDs. `arcium test` starts local Arx nodes in Docker and runs the end-to-end test.
...
**Test hangs at `awaitComputationFinalization`** -- Localnet Arx containers may have crashed. Inspect: `docker ps | grep arx`. If unhealthy: `docker stop $(docker ps -q --filter "name=arx-")`, then re-run `arcium test`. Note: `arcium clean` does NOT stop Docker containers.

**Need a bigger mempool?** -- Default is `medium`. Pass `--mempool-size large` (or `tiny|small|medium|large`) to `arcium test`. Localnet only.

## Documentation

- [Core Concepts](https://docs.arcium.com/developers/core-concepts) -- MXE, clusters, computations, and how they connect
- [Mental Model](https://docs.arcium.com/developers/arcis/mental-model) -- how Arcium works
